...
<div class="card border-top-primary shadow h-100">
    <div class="card-body">
        <h3>{{__('lang.todayAppointment')}} : {{$countme}}</h3>
        <hr class="mb-0">
    ...
    <li class="list-group-item">
        <div class="row justify-content-center">
            <div style="display: table; overflow: hidden;" class="col-lg-1 col-md-4 col-sm-4">
                ...
                <a class="btn btn-outline-success btn-block" href="{{ route('appointment.edit', $appointment->id) }}"><i class="fa fa-edit"></i></a>
                </div>
            </div>
            <div style="display: table; overflow: hidden;" class="col-lg-4 col-md-4 col-sm-4 marginTopResponsive">
                <div style="display: table-cell; vertical-align: middle;">
                   <h5> <a href="patient/{{ $appointment->patient[0]->id }} "> {{$appointment->patient[0]->firstname.' '.$appointment->patient[0]->midname.' '.$appointment->patient[0]->lastname}} </a></h5>
                </div>
            </div>
            ...
            <div class="text-center btn-group btn-block marginTopResponsive {{-- @if (app()->getlocale()=='ar') float-left @else float-right @endif  --}}" style="direction: ltr !important">
                <button id="ChangeAfterSuccess" data-patient_id="{{$appointment->patient[0]->id}}"
                    data-appointment_id="{{$appointment->id}}"
                    class="OpenModal  btn btn-outline-primary text-center justify-content-center mb-0 my-0 success-change"
                    type="button"><i class="fas fa-check"></i> {{ __('lang.MarkDone') }}</button>
            <button  data-appointmentid="{{$appointment->id}}"  type="button" class="btn btnAbort  btn-outline-danger text-center justify-content-center mb-0 my-0"><i class="fas fa-times"></i> {{__('lang.Abort')}}</button>
           </div>
