                ...
                <div class="card-body message-box" wire:poll="mountdata">
                   @if(filled($allmessages))
                     @foreach($allmessages as $mgs)
                            <div data-msg-id="" class="single-message @if($mgs->user_id == auth()->id()) sent @else received @endif">
                                @if($mgs->user_id == auth()->id())
                             <span class="mb-4">{{ $mgs->message}}</span> <br>@else
                             {{ $mgs->message}}<br>@endif
                             @if($mgs->user_id == auth()->id())
                              @if($mgs->is_seen==false) <small><span class="border-top"> {{ $mgs->created_at }} <i class="ml-2 far fa-check-circle"></i></span></small> @endif @if($mgs->is_seen==true) <small><span class="border-top">{{ $mgs->created_at }} <i class="ml-2 fas fa-check-circle"></i></span></small> @endif @endif
                            </div>  
